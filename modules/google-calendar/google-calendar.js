...
const db = {
  getCalendarSettings: async () => {
    // Replace this with your database logic to retrieve calendar settings.
    return null;
  },
};
...
// Helper function to get calendar settings from the database
const getCalendarSettings = async () => {
  try {
    const settings = await db.getCalendarSettings();
...
export async function createAvailabilityEvent(scheduleData) {
  try {
    ...
    // Parse time from the time slot for pickup events
    let startDateTime = new Date(scheduleData.date);
    let endDateTime = new Date(scheduleData.date);
    
    if (scheduleData.type === 'pickup' && scheduleData.slots.length > 0) {
      // Parse the time from the first slot (format like "9am-3pm")
      const timeSlot = scheduleData.slots[0].time;
      const timeMatch = timeSlot.match(/(\d+(?::\d+)?(?:am|pm))-(\d+(?::\d+)?(?:am|pm))/i);
      ...
      if (timeMatch) {
        ...
        // Parse start time
        const startHourMatch = startTimeStr.match(/(\d+)(?::(\d+))?(?:am|pm)/i);
        let startHour = parseInt(startHourMatch[1]);
        const startMinute = startHourMatch[2] ? parseInt(startHourMatch[2]) : 0;
        ...
        // Parse end time
        const endHourMatch = endTimeStr.match(/(\d+)(?::(\d+))?(?:am|pm)/i);
        let endHour = parseInt(endHourMatch[1]);
        const endMinute = endHourMatch[2] ? parseInt(endHourMatch[2]) : 0;
        
        // Adjust for AM/PM
        if (endTimeStr.toLowerCase().includes('pm') && endHour < 12) {
          endHour += 12;
        }
        ...
        console.log(`Parsed pickup hours: ${startHour}:${startMinute} to ${endHour}:${endMinute}`);
      } else {
        // Default hours if we can't parse the time
        console.log(`Could not parse time format "${timeSlot}", using default hours`);
        startDateTime.setHours(9, 0, 0, 0); // 9 AM
        endDateTime.setHours(17, 0, 0, 0);  // 5 PM
      }
    } else {
      // For delivery events or if no time slot specified, use default time span
      endDateTime.setTime(startDateTime.getTime() + 8 * 60 * 60 * 1000); // 8 hours later
    }
...
export async function createPickupCalendarEvent(orderData) {
  ...
  try {
    ...
    // If this is a real order in Firestore, update the pickup time there too
    if (orderData.id && orderData.id.startsWith('order-') && process.env.NODE_ENV === 'production') {
      try {
        ...
        await firestore.collection('orders').doc(orderData.id).update({
          'pickupInfo.time': `${eventStartTime} - ${eventEndTime}`,
          updatedAt: admin.firestore.Timestamp.fromDate(new Date())
        });
        console.log(`Updated pickup time in Firestore for order ${orderData.id}`);
      } catch (updateError) {
        console.error(`Failed to update pickup time in Firestore: ${updateError.message}`);
      }
    }
